<?php
function al_pricing_tables_scripts($hook) {
	
	if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'pricing_tables') {
		// Admin Stylesheet
		//wp_enqueue_style('al-pricing-tables', get_bloginfo('stylesheet_directory') . '/library/admin/css/widgets-area.css', false, '1.0.0', 'screen');
		// JAVASCRIPT
		//wp_enqueue_script('table-copy', get_template_directory_uri . '/library/admin/js/jquery.zclip.min.js', array('jquery'), '1.0.4');
	}
}

add_action('admin_enqueue_scripts', 'al_pricing_tables_scripts');
...
function pricing_tables()
{
	...
	<div id="header">
        <div id="logo-container">
			<h1><img src="<?php echo get_template_directory_uri()?>/library/admin/images/main_logo.png" alt="TouchM" /></h1>		
        </div>	
        <div id="admin-panel">
		      	...
		      	<form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>?page=pricing_tables&amp;action=<?php echo $wizard->getStepName() ?>" id="custom" class="form">
			        ...
			        <fieldset class="step">
				        ...
				        <div style="float:left; width:200px;  margin-left:10px">
				    		...
				    		<label for="style1" style="display:inline; width:122px; margin-top:0px">Narrow size</label> 
				    		<input type="radio" style="width:20px; padding-top:10px" name="style" value="style1" id="style1" <?php if ($wizard->getValue('style') == 'style1'):?>checked="checked"<?php endif?> />
						</div>
						<div style="float:left; width:200px; margin-left:70px">
							<p><img src="<?php echo get_template_directory_uri()  ?>/library/admin/images/pricing-tables/table2.png"  alt="Style1" /> </p>
				    		<label for="style2" style="display:inline; width:122px; margin-top:0px">Regular size - type 1</label> 
				    		<input type="radio" style="width:20px; padding-top:10px" name="style" value="style2" id="style2" <?php if ($wizard->getValue('style') == 'style2'):?>checked="checked"<?php endif?> />
						</div>
						<div style="float:left; width:200px; margin-left:70px">
							<p><img src="<?php echo get_template_directory_uri()  ?>/library/admin/images/pricing-tables/table3.png"  alt="Style1" /> </p>
				    		<label for="style3" style="display:inline; width:122px; margin-top:0px">Regular size - type 2</label> 
				    		<input type="radio" style="width:20px; padding-top:10px" name="style" value="style3" id="style3" <?php if ($wizard->getValue('style') == 'style3'):?>checked="checked"<?php endif?> />
						</div>
